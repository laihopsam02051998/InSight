...
const SubItem: React.FC<IMenu> = ({data, activePath}: IMenu) => {
  const item = data;
  const _pathnameSplit = activePath.split("/");
    ...
    <Menu mode='vertical' className="dropdown-3dot" >
      {item.routes.length > 0 &&
        item.routes.map((linkNav: IRouter, index) => {
          ...
          } else if (matchPath(activePath, { path: linkNav.path, exact: linkNav.exact })) {
            active = "ant-menu-item-selected"
          }
...
const Item: React.FC<IMenu> = ({data}: IMenu) => {
  const item = data;
